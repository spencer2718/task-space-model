...
O*NET provides 2,087 Detailed Work Activities (DWAs) linked to 894 O*NET-SOC occupations with DWA data \citep{onet2024dwa,onet2024scales}. We represent each occupation $j$ as a probability distribution $\rho_j$ over activities: importance-weighted, filtered for relevance, and normalized. The distribution is sparse---the median occupation uses 19 of 2,087 DWAs.
...
\begin{tabular}{llccc}
\toprule
\textbf{Specification} & \textbf{Note} & $\alpha$ & Pseudo-$R^2$ & $\Delta$ vs baseline \\
\midrule
Embedding $\times$ Centroid & (primary) & 7.40 & 14.08\% & +132\% \\
Embedding $\times$ Wasserstein & & 8.39 & 13.8\% & +127\% \\
O*NET $\times$ Cosine & & 4.55 & 8.05\% & +33\% \\
O*NET $\times$ Euclidean & & 9.76 & 6.06\% & baseline \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Embedding $\times$ Centroid vs.\ Embedding $\times$ Wasserstein & 0.95 \\
Embedding $\times$ Centroid vs.\ O*NET $\times$ Cosine & 0.47 \\
Embedding $\times$ Centroid vs.\ Wasserstein (identity) & 0.35 \\
Embedding $\times$ Centroid vs.\ O*NET $\times$ Euclidean & 0.23 \\
\bottomrule
\end{tabular}
\end{table}


The two embedding-based distance matrices are highly correlated ($\rho = 0.95$; \cref{tab:distance-correlations}), while their correlation with O*NET-based or identity-based measures is low ($\rho = 0.23$--$0.47$). Centroid averaging inherits the embedding structure rather than the task-overlap structure.
...
The decomposition in \cref{tab:demand-decomposition} clarifies the scope of this ranking performance. Aggregate inflows are demand-dominated ($\rho = 0.80$ with BLS openings). The distance measure's contribution is to within-origin destination ranking (per-origin $\rho \approx 0.12$ on the 2024 out-of-period comparison, $n = 233$ origins). The measure is informative about which destinations are skill-compatible for workers leaving a given origin, but not about which destinations receive the largest aggregate inflows.
...
\midrule
\multicolumn{3}{l}{\textit{Out-of-period comparison ($n = 8{,}880$):}} \\
...
Task distance explains a modest share of aggregate bilateral flow variation (\cref{tab:gravity-comparison}). The Embedding $\times$ Centroid specification achieves partial $R^2 = 2.55\%$ beyond employment mass terms (mass-only $R^2 = 22.06\%$); Embedding $\times$ Wasserstein achieves 3.46\%. Both are consistent with the \citet{CortesGallipoli2018} finding that task-related costs account for a small share of total transition costs.
...
The results establish scope conditions for the proposed measure. First, it is informative about within-origin destination ranking ($\rho \approx 0.12$) but not aggregate allocation. Aggregate inflows are demand-dominated ($\rho = 0.80$); the distance measure does not incorporate vacancy, wage, or geographic information. Applications that require aggregate reallocation forecasts would need to combine distance with demand-side data.

Second, the measure captures occupational proximity for mobility, not automation susceptibility. It is orthogonal to routine task intensity ($r = -0.06$) and empirically distinct from AI exposure ($r = -0.02$). These are different constructs: exposure measures identify which occupations are affected by technology; distance measures characterize which transitions are skill-compatible.
...
As an interim approach, we anchor externally to \citet{DixCarneiro2014}'s median sectoral switching cost of 2.0 wage-years. With median centroid distance 0.229, this implies 8.74 wage-years per unit centroid distance; equivalently, with median Wasserstein distance 0.521, 3.84 wage-years per unit Wasserstein distance. US estimates in the literature range from 0.5$\times$ to 6.5$\times$ annual wages; ordinal rankings of transition difficulty are invariant across this range by construction (the calibration is linear). These magnitudes should be treated as rough planning numbers, not identified structural parameters.
