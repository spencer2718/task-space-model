...
The choice between continuous and discrete propagation is empirical. If kernel-weighted overlap predicts outcomes better than binary overlap, the continuous mechanism adds value. If not, the simpler discrete mechanism is preferred. Phase~I validation (\cref{subsec:phase1}) tests this by comparing kernel-weighted occupation-pair overlap against binary Jaccard overlap as predictors of wage comovement. The result: continuous overlap outperforms discrete when the kernel is properly calibrated.
\end{remark}
...
\subsection{Why Continuous Structure May Dominate}
...
\subsection{Why Discrete Structure Might Dominate (Falsified)}
...
The threshold model predicts: (i) binary activity overlap should predict wage comovement, (ii) continuous distance weighting should not improve prediction beyond binary overlap, and (iii) the spillover bandwidth $\sigma$ should be inert. Phase~I falsifies predictions (ii) and (iii): continuous overlap significantly outperforms binary overlap when properly calibrated, and $\sigma$ matters (calibrating to nearest-neighbor vs global distances produces different results). The threshold model is rejected in favor of the continuous manifold representation.
\end{remark}
...
\hline
\end{tabular}
\caption{Validation targets by activity-distance recipe and Phase~I outcomes. Continuous semantic overlap outperforms binary overlap in explanatory power ($R^2$) when properly calibrated.}
...
Normalized kernel-weighted overlap significantly outperforms binary Jaccard overlap in explaining wage comovement. The improvement in $R^2$ is 191\% (from 0.00167 to 0.00485). This is not driven by concentration effects: after controlling for entropy (specialist vs generalist) and support size (number of activities), the semantic overlap coefficient remains highly significant ($t = 5.29$).
...
An earlier version (v0.5.0) concluded that ``discrete dominates continuous'' based on kernel-weighted overlap failing permutation testing. This was traced to bandwidth miscalibration: setting $\sigma$ to the global distance median (0.744) caused kernel collapse, making all continuous measures equivalent to noise. With proper calibration ($\sigma = 0.223$, the NN median), the continuous measure substantially outperforms the discrete baseline.
...
\hline
\end{tabular}
\caption{Historical regimes as parameter signatures. ``Mechanism'' column records whether continuous or discrete activity structure is the primary channel; Phase~I suggests continuous dominates when properly calibrated.}
\label{tab:regimes}
\end{table}
...
The central empirical finding is that continuous semantic structure significantly improves prediction of wage comovement when the kernel is properly calibrated. Normalized kernel-weighted overlap achieves $t = 7.14$ (clustered standard errors), $R^2 = 0.00485$---a 191\% improvement in explanatory power over binary Jaccard overlap ($t = 8.00$, $R^2 = 0.00167$). The semantic signal is robust: it survives permutation testing ($p < 0.001$), remains significant after controlling for occupational concentration ($t = 5.29$ with entropy controls), and achieves the 100th percentile against random baseline distributions.
...
The framework's contribution is a disciplined measurement pipeline that reveals what kind of structure actually matters for economic outcomes. The finding that continuous semantic similarity outperforms discrete activity classification supports the manifold representation of the task domain and suggests that kernel-weighted exposure measures can capture economically meaningful variation that binary classification misses. The methodological lessons---calibrate bandwidth to local structure, normalize for concentration, validate against random baselines---provide guidance for future implementations.
