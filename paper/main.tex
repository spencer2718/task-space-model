...
\begin{abstract}
We propose a modular architecture---the \emph{task-space oracle}---as infrastructure for analyzing how task-level technological change propagates to occupation-level employment and wages. The oracle has four modules: $\mathcal{T}$ (task representation), $\mathcal{I}$ (institutional structure), $\mathcal{S}$ (shock profiles), and $\mathcal{M}$ (adjustment mechanisms). A complete oracle maps $(\mathcal{T}, \mathcal{I}, \mathcal{S}, \mathcal{M}) \to (\Delta\boldsymbol{\rho}, \Delta \mathbf{L}, \Delta \mathbf{W})$: from inputs to occupation-specific changes in task distributions, employment, and wages.

Our research validates initial module specifications and quantifies the supply-demand decomposition of occupational transitions. The $\mathcal{T}$ module is validated: Wasserstein geometry substantially outperforms kernel overlap for predicting worker transitions ($\Delta \text{log-likelihood} = +9{,}576$), confirming that workers minimize skill transformation cost. The $\mathcal{I}$ module is validated: institutional distance adds predictive power conditional on semantic distance ($t = 34.6$). The $\mathcal{S}$ module is integrated: external AI exposure scores are orthogonal to task geometry ($r = 0.02$), confirming they capture distinct constructs. The $\mathcal{M}$ module is calibrated externally: 3.84 wage-years per unit Wasserstein distance, anchored to structural estimates.

A central finding is the supply-demand decomposition: aggregate occupational inflows are demand-dominated ($\rho = 0.80$ for job openings alone), while geometry captures supply-side pathway direction (per-origin $\rho \approx 0.13$). This decomposition validates the oracle architecture's separation of where workers \emph{can} go (supply-side feasibility from skill geometry) from where they \emph{do} go (demand-side realization). Validation uses 89,329 CPS occupation transitions. The modular architecture enables cumulative progress; this paper establishes that the architecture is viable and empirically grounded.
\end{abstract}
...
The paper's contributions are framed as module validation checkpoints. First, the $\mathcal{T}$ module is validated: Wasserstein distance---measuring the minimum cost to transform one occupation's skill portfolio into another---substantially outperforms kernel overlap for predicting worker mobility ($\Delta \text{log-likelihood} = +9{,}576$ on $n = 89{,}329$ transitions). This validates the ``earth mover's'' interpretation of occupational distance: workers minimize skill transformation cost when switching occupations.
...
Fifth, scope is empirically quantified through supply-demand decomposition. Aggregate occupational inflows are demand-dominated: job openings alone predict aggregate inflows with $\rho = 0.80$, while geometry alone achieves $\rho = 0.04$. For pathway direction at the origin level, geometry provides modest but statistically significant information (per-origin $\rho \approx 0.13$, $n = 233$ origins). This decomposition validates the oracle architecture's separation of supply-side feasibility (where workers \emph{can} go given skill geometry) from demand-side realization (where they \emph{do} go given labor market conditions). The framework measures structural feasibility---the supply-side input to equilibrium analysis.
...
Our approach complements structural estimation \citep{Traiberman2019} by trading parametric richness for applicability when administrative data is unavailable. We extend task-distance approaches \citep{GathmannSchonberg2010,CortesGallipoli2018} with optimal transport geometry. The supply-demand decomposition ($\rho \approx 0.80$ demand, $\rho \approx 0.13$ geometry) is consistent with \citet{CortesGallipoli2018}'s finding that task-related costs explain approximately 15\% of total transition frictions. The framework provides infrastructure for cumulative progress: validated modules can be reused as new capabilities are added.


\Cref{sec:literature} reviews related literatures organized by oracle module. \Cref{sec:theory} develops the oracle architecture and module specifications. \Cref{sec:data} operationalizes each component using O*NET data. \Cref{sec:validation} reports module validation results including geometry comparison, CPS mobility, shock integration, cost calibration, and supply-demand decomposition. \Cref{sec:extensions} documents extensions, completed investigations, and deferred work. \Cref{sec:limitations} documents scope conditions. \Cref{sec:applications} discusses policy applications and the research roadmap. \Cref{sec:conclusion} concludes with module status and next steps.
...
Gravity models of labor flows provide benchmarks for predictive accuracy. \citet{CortesGallipoli2018} use US CPS aggregate flows with DOT task measures, finding task-related costs explain $\leq$15\% of total transition costs. \citet{SiminiEtAl2021} achieve correlations of 0.3--0.5 for geographic mobility predictions. These benchmarks contextualize our supply-demand decomposition.
...
\textbf{CPS Transitions:} IPUMS CPS Basic Monthly Files, 2015-01 through 2024-09 (excluding 2020--2021 for COVID disruption). Training sample: 2015--2019, 2022--2023 ($n = 97{,}236$). Holdout: 2024 ($n = 8{,}880$).
...
\textbf{Data:} $n = 89{,}329$ verified CPS occupation transitions (2015--2019, 2022--2024). Persistence filter applied to remove measurement error.
...
\textbf{Sample:} Training $n = 97{,}236$; holdout $n = 8{,}880$ (2024 transitions).
...
\begin{itemize}
    \item \textbf{Per-origin pathway direction:} For each origin occupation, correlate model-predicted destination rankings (based on inverse Wasserstein distance) with observed destination rankings, then average across origins.
...
\textbf{Metric} & \textbf{Value} & \textbf{Interpretation} \\
\midrule
N observed transitions & 8,880 & Holdout sample \\
N origins evaluated & 233 & Full sample \\
\textbf{Per-origin Spearman $\rho$} & \textbf{0.128} & Modest pathway direction signal \\
...
\textbf{Interpretation:} The decomposition reveals that aggregate occupational inflows are overwhelmingly demand-determined ($\rho = 0.80$). Geometry-based feasibility provides modest but statistically significant information about pathway direction at the origin level ($\rho \approx 0.13$), consistent with \citet{CortesGallipoli2018}'s finding that task-related costs explain approximately 15\% of total transition frictions. The oracle architecture's separation of supply-side geometry ($\mathcal{T}$, $\mathcal{I}$ modules) from demand-side mechanisms ($\mathcal{M}$ module) is empirically validated: geometry defines the feasible choice set, while demand determines selection within that set. This parallels the distinction between consideration sets and revealed choice in discrete choice models \citep{BarseghyanEtAl2021} and supply-demand decompositions in structural labor economics \citep{LeeWolpin2006}.

\textbf{Why Top-5 = 0:} Historical baselines concentrate mass on modal destinations (32.1\% top-5 accuracy). Geometry spreads probability across \emph{feasible} destinations. With demand dominating aggregate flows ($\rho = 0.80$), zero top-5 overlap is expected---geometrically central destinations are not necessarily high-demand destinations. Census code 2050 (``Directors, religious activities'') was predicted as a top destination by geometry but received only 6 actual inflows. When historical patterns break down (technological disruption, new occupations), geometry-based feasibility is the appropriate input for counterfactual analysis.
...
\begin{itemize}
    ...
    \item Supply-demand decomposition (\cref{subsec:pathway-validation}): Demand $\rho = 0.80$, geometry $\rho = 0.13$
\end{itemize}
...
\textbf{Status: COMPLETED.} Demand-only $\rho = 0.798$ for aggregate inflows; geometry-only $\rho = 0.043$ for aggregate; per-origin geometry $\rho = 0.128$ for pathway direction.

\textbf{Implication:} Aggregate flows are demand-dominated. Geometry captures supply-side constraints (necessary condition for transition), not demand-side outcomes (sufficient condition). The oracle architecture's separation of feasibility from realization is empirically validated.
...
The supply-demand decomposition (\cref{subsec:pathway-validation}) quantifies this limitation: demand-only $\rho = 0.80$ for aggregate inflows demonstrates that supply-side feasibility alone cannot predict realized flows. Geometry provides modest information about pathway direction (per-origin $\rho \approx 0.13$) but negligible information about aggregate destinations (geometry-only $\rho = 0.04$). This is both a limitation and a finding---the decomposition validates the scope claim while quantifying what demand integration would add.
...
The framework does \emph{not} support: point predictions of labor reallocation, forecasts of which jobs will be created/destroyed, or capacity for absorption without demand data. The supply-demand decomposition ($\rho \approx 0.80$ demand, $\rho \approx 0.13$ geometry) quantifies why: realized flows depend heavily on labor market conditions, not just skill feasibility.
...
\begin{enumerate}
    ...
    \item \textbf{Scope:} Empirically quantified. Geometry captures supply-side pathway direction (per-origin $\rho \approx 0.13$); demand dominates aggregate flows ($\rho \approx 0.80$). This decomposition validates the oracle architecture's separation of feasibility from realization.
\end{enumerate}
...
\begin{itemize}
    ...
    \item Aggregate flows are demand-dominated ($\rho \approx 0.80$); geometry captures supply-side constraints ($\rho \approx 0.13$)
\end{itemize}


\textbf{The research program continues.} The task-space oracle is an organizing architecture, not a deliverable. This paper establishes that the architecture is viable: validated components ($\mathcal{T}$, $\mathcal{I}$) can be combined with external shock profiles ($\mathcal{S}$) to characterize structural feasibility. The supply-demand decomposition provides the key insight from this phase: the architecture's separation of supply-side feasibility from demand-side mechanisms is empirically validated. Geometry defines the feasible choice set; demand determines selection within that set. Future work extends $\mathcal{M}$ (equilibrium mechanisms, demand integration) and refines $\mathcal{S}$ (modality-specific profiles). The modular structure enables cumulative progress toward complete labor market modeling under technological change.
