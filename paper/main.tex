...
\textbf{Module status.} $\mathcal{T}$: Validated---embedding-informed distance achieves 13.8--14.1\% pseudo-$R^2$ on 89,329 CPS transitions; the embedding ground metric (+83\% vs.\ identity) drives improvement, not distributional treatment (Wasserstein adds nothing beyond centroid). We find that most of the predictive gain comes from the embedding representation of inter-task similarity, not from the distributional treatment of occupations. Embedding-based specifications achieve pseudo-$R^2$ of 13.8--14.1\%, compared to 6.1--8.1\% for O*NET-based specifications. $\mathcal{I}$: Validated---institutional distance adds predictive power conditional on $\mathcal{T}$ ($t = 34.6$). $\mathcal{S}$: Validated---external AI exposure scores are orthogonal to geometry ($r = 0.02$); geometry substantially outperforms historical baselines ($\Delta\text{LL} = +23{,}119$ on holdout). $\mathcal{M}$: Calibrated externally (3.84 wage-years per unit distance).
...
\begin{itemize}
    \item \textbf{Embedding effect:} $\sim$70\% improvement in pseudo-$R^2$ (embedding vs.\ O*NET methods)
...
\begin{itemize}
    \item \textbf{Embedding effect:} Embedding-based methods (13.8--14.1\% pseudo-$R^2$) vs.\ O*NET-based methods (6.1--8.1\%) represents a $\sim$70\% relative improvement. This is the dominant effect.
...
Embedding (semantic similarity) & 8.39 & 0.154 & 13.8\% & +83\% \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
$\alpha$ (semantic) & 7.400 & 0.036 & 206.5 & $< 10^{-100}$ \\
$\beta$ (institutional) & 0.154 & 0.004 & 34.6 & $< 10^{-100}$ \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Interpretation:} Predicted destinations are skill-compatible but credential-gated or capacity-constrained. The ratio $\gamma_{\mathrm{inst}}/\gamma_{\mathrm{sem}} = 0.021$ means credentials barely register---because blocked transitions are unobserved. This is selection bias inherent in revealed preference estimation (\cref{rem:friction-vs-gates}), not model failure.
...
\midrule
$\mathcal{T}$ & Operational & Embedding $\gg$ O*NET ($\sim$70\%) & Domain-specific embeddings \\
...
\midrule
Pre-COVID (2015--2019) & 7.40 & 0.044 & 0.162 & 0.005 & 14.1\% \\
Post-COVID (2022--2024) & 7.35 & 0.064 & 0.161 & 0.007 & 13.9\% \\
\bottomrule
\end{tabular}
\end{table}


\textbf{Aggregate stability:} The likelihood ratio test for structural break yields $\chi^2(2) = 0.54$, $p = 0.76$---we cannot reject parameter equality across periods. The task-distance penalty ($\alpha$) changes by less than 1\% (7.40 $\to$ 7.35). This invariance across the largest labor market disruption in decades supports the interpretation that embedding-based distance captures structural skill transferability constraints rather than contingent revealed preferences. This finding is consistent with \citet{PizzinelliShibata2023}'s evidence that aggregate occupational mismatch remained stable through COVID despite massive employment disruption.
...
\begin{itemize}
    \item 2$\times$2 methodology comparison (\cref{subsec:geometry-comparison}): Embedding effect ($\sim$70\%) dominates distributional effect (centroid marginally outperforms Wasserstein)
...
\textbf{Status: RESOLVED.} Embedding effect ($\sim$70\%) dominates distributional effect (centroid marginally outperforms Wasserstein). The embedding ground metric (semantic task substitutability) is the validated mechanism.
...
\midrule
Embedding $\gg$ O*NET & Semantic task substitutability matters & \textbf{Confirmed ($\sim$70\%)} \\
...
\textbf{Status: DEFERRED.} With embedding-informed distance validated as primary geometry, domain-specific comparison is lower priority. The embedding effect ($\sim$70\%) is large; further embedding optimization unlikely to produce comparable improvement.
...
\textbf{Module status:}
\begin{enumerate}
    \item \textbf{$\mathcal{T}$ module (task representation):} Validated. Embedding-informed distance substantially outperforms O*NET-based measures ($\sim$70\% in pseudo-$R^2$). The mechanism is semantic task substitutability: sentence embeddings capture that similar tasks are interchangeable even when distinctly labeled. Wasserstein optimal transport provides theoretical grounding (minimum transformation cost) but does not outperform simpler centroid methods after diagonal bias correction; centroid-based distance is marginally preferred ($\rho = 0.95$ correlation with Wasserstein).
...
\begin{itemize}
    \item Embedding ground metric matters: semantic task substitutability drives $\sim$70\% improvement over O*NET baselines
