...
The paper's contributions are framed as module validation checkpoints. First, the $\mathcal{T}$ module is validated: Wasserstein distance---measuring the minimum cost to transform one occupation's skill portfolio into another---substantially outperforms kernel overlap for probabilistic scoring of realized transitions (median $\Delta \text{log-likelihood} = +13{,}052$ across 81 multiverse specifications). This is consistent with the ``earth mover's'' interpretation of occupational distance: workers minimizing skill transformation cost when switching occupations.
...
\textbf{Theoretical prediction:} If workers' reallocation costs correspond to skill transformation difficulty, Wasserstein should outperform kernel for mobility scoring. If ground metric is noisy (text embeddings imperfectly capture economic substitutability), kernel's implicit regularization may dominate.

\textbf{Empirical resolution:} Wasserstein substantially outperforms kernel overlap (median $\Delta \text{log-likelihood} = +13{,}052$; see \cref{subsec:geometry-comparison}). Among two plausible geometries built on the same ground metric, the one that respects metric structure (Wasserstein) better matches mobility patterns in our setting. This is consistent with the earth mover's interpretation, though the reduced-form design cannot distinguish transformation cost from correlated mechanisms.
\end{remark}
...
\begin{enumerate}
    \item \textbf{Representation axis:} How to represent tasks (MPNet embeddings, MiniLM, DistilRoBERTa)
...
Activity distance & Text embedding (MPNet) & MiniLM, DistilRoBERTa \\
\textbf{Occupation distance} & \textbf{Wasserstein} & Kernel overlap \\
Kernel bandwidth & Median NN distance & Fixed (0.15, 0.30) \\
...
\textbf{Objective:} Test whether optimal transport (Wasserstein) outperforms smoothed intersection (kernel overlap) for probabilistic scoring of realized transitions. This resolves a key model space question for the $\mathcal{T}$ module.
...
\begin{table}[h!]
\centering
\caption{Geometry Comparison: Wasserstein vs.\ Kernel Overlap (Multiverse Summary)}
\label{tab:geometry-comparison}
\begin{tabular}{lccc}
\toprule
\textbf{Metric} & \textbf{Value} & \textbf{Distribution} & \textbf{Interpretation} \\
\midrule
Specifications tested & 81 & --- & Full multiverse \\
Wasserstein win rate & 100\% & 81/81 & No kernel dominance \\
$\Delta$LL median & $+13{,}052$ & [P5: $+2{,}720$, P95: $+48{,}046$] & Substantial improvement \\
$\Delta$LL mean & $+17{,}930$ & (std: 15,115) & --- \\
\bottomrule
\end{tabular}
\end{table}


\textbf{Results:} Wasserstein geometry outperforms kernel-based distance in 100\% of multiverse specifications (81/81). Median improvement $\Delta \text{LL} = +13{,}052$ [P5: $+2{,}720$, P95: $+48{,}046$]. The improvement far exceeds pre-committed thresholds across all tested configurations.

Robustness holds across embedding models (MPNet, MiniLM, DistilRoBERTa), bandwidth calibrations (data-driven and fixed), sample periods (pre-COVID, post-COVID, full), and distance thresholds. No specification produces kernel dominance.

\textbf{Exploratory finding:} Excluding long-distance transitions amplifies Wasserstein advantage (mean $\Delta$LL increases from $+7{,}724$ with no threshold to $+30{,}960$ at P95 threshold). This pattern is consistent with geometry capturing feasibility-constrained transitions, while long-distance moves reflect demand shocks or idiosyncratic factors outside the geometric framework.

The institutional coefficient changes across geometry ($-49\%$ from kernel to Wasserstein). The modest correlation between task distance and institutional distance ($r = 0.25$) is consistent with---but does not establish---that Wasserstein absorbs training-related variance, leaving a residual institutional component reflecting non-skill barriers. An interaction test (institutional effect $\times$ destination licensing intensity) would provide stronger evidence; we note this as a robustness check for future work.

\textbf{Module status: $\mathcal{T}$ VALIDATED.} Wasserstein is adopted as primary geometry. Metric structure matters for mobility scoring. The pattern is consistent with the ``earth mover's'' interpretation: workers minimizing skill transformation cost when switching occupations.
...
\midrule
$\mathcal{T}$ & Operational & Wasserstein $\gg$ kernel (81/81) & Embedding comparison \\
...
\begin{itemize}
    \item Wasserstein vs.\ kernel overlap comparison (\cref{subsec:geometry-comparison}): Wasserstein substantially outperforms (median $\Delta \text{LL} = +13{,}052$, 81/81 specifications)
    ...
    \item Multiverse robustness (\cref{subsec:geometry-comparison}): 100\% win rate across 81 specifications
\end{itemize}
...
\textbf{Objective:} Test whether Wasserstein distance outperforms kernel overlap for mobility scoring.

\textbf{Status: RESOLVED.} Wasserstein substantially outperforms kernel overlap (median $\Delta \text{log-likelihood} = +13{,}052$, 100\% win rate across 81 specifications).
...
\textbf{Threshold exceeded:} Median $\Delta \text{log-likelihood} = +13{,}052 \gg 100$ (threshold). Mean $\Delta \alpha = +57\% \gg 10\%$ (threshold).

\textbf{Implication:} Wasserstein is adopted as primary semantic distance measure. The pattern is consistent with the ``earth mover's'' interpretation of occupational distance.
...
\textbf{Status: DEFERRED.} With Wasserstein validated as primary geometry, embedding comparison is lower priority. The geometry change (kernel $\to$ Wasserstein) produced median $\Delta \text{LL} = +13{,}052$. The embedding change is unlikely to produce comparable improvement.
...
\textbf{Implication:} Aggregate flows are demand-dominated. Geometry captures supply-side constraints (necessary condition for transition), not demand-side outcomes (sufficient condition). The oracle architecture's separation of feasibility from realization is consistent with observed patterns.
...
The demand decomposition clarifies the framework's operational role. Under stable conditions, flows are demand-dominated ($\rho = 0.80$); geometry provides modest directional signal ($\rho \approx 0.13$). Under disruption---when historical demand patterns break---geometry becomes the stable input. The framework measures where workers \emph{can} go (supply-side feasibility), not where they \emph{do} go (demand-side realization). This is the operational use case: counterfactual analysis when past flows are uninformative.
...
\textbf{Module status:}
\begin{enumerate}
    \item \textbf{$\mathcal{T}$ module (task representation):} Validated. Wasserstein geometry substantially outperforms kernel overlap (median $\Delta \text{LL} = +13{,}052$, 100\% win rate across 81 specifications). The pattern is consistent with workers minimizing skill transformation cost when switching occupations.
    
    \item \textbf{$\mathcal{I}$ module (institutional structure):} Validated. Institutional distance is associated with mobility conditional on semantic distance ($t = 34.6$). Separability holds. Directional asymmetry is sample-dependent---an informative constraint.
    ...
    \item \textbf{Scope:} Empirically quantified. Geometry captures supply-side pathway direction (per-origin $\rho \approx 0.13$); demand dominates aggregate flows ($\rho \approx 0.80$). This decomposition is consistent with the oracle architecture's separation of feasibility from realization.
\end{enumerate}


\textbf{Informative constraints from this phase:}
\begin{itemize}
    \item Metric structure matters: Wasserstein $\gg$ kernel (81/81 specifications)
...
\textbf{The research program continues.} The task-space oracle is an organizing architecture, not a deliverable. This paper establishes that the architecture is viable: validated components ($\mathcal{T}$, $\mathcal{I}$) can be combined with external shock profiles ($\mathcal{S}$) to characterize structural feasibility. The supply-demand decomposition provides the key insight from this phase: the architecture's separation of supply-side feasibility from demand-side mechanisms is consistent with observed patterns. Geometry defines the feasible choice set; demand determines selection within that set. Future work extends $\mathcal{M}$ (equilibrium mechanisms, demand integration) and refines $\mathcal{S}$ (modality-specific profiles). The modular structure enables cumulative progress toward complete labor market modeling under technological change.
