...
\textbf{Module status: $\mathcal{S}$ VALIDATED.} External shock profiles and feasibility geometry combine coherently. The orthogonality confirms complementary constructs; holdout performance confirms predictive validity.
...
$\mathcal{S}$ & Validated & AIOE linkage; $\Delta$LL = +23,119 & Modality-specific profiles \\
...
\begin{enumerate}
    ...
    \item \textbf{$\mathcal{S}$ module (shock profiles):} Validated. External AIOE scores are orthogonal to geometry ($r = 0.02$), confirming distinct constructs that combine coherently. Geometry substantially outperforms historical baselines on holdout data ($\Delta\text{LL} = +23{,}119$).
    ...
    \item \textbf{Scope:} Empirically quantified. Geometry captures supply-side pathway direction (per-origin $\rho \approx 0.13$); demand dominates aggregate flows ($\rho \approx 0.80$). Gravity model confirms task distance explains $\sim$3.5\% of aggregate bilateral flow variation, consistent with \citet{CortesGallipoli2018}. This decomposition is consistent with the framework architecture's separation of feasibility from realization.
\end{enumerate}
