...
Sentence embeddings have been applied to occupation and task descriptions in prior work, but validation typically uses O*NET's own similarity measures or expert ratings---not observed worker behavior. We validate embedding-based occupation distance against 89,329 CPS transitions and find substantial improvement over traditional O*NET-based measures (+75\% in pseudo-$R^2$). A 2$\times$2 methodology comparison isolates the source: the embedding representation, not the aggregation method (Wasserstein vs.\ centroid), drives the improvement; the two embedding-based approaches produce nearly identical distance rankings ($\rho = 0.95$). For aggregate bilateral flows, task distance explains a modest share of variation ($\sim$3.5\% partial $R^2$), consistent with prior structural estimates that task-specific costs comprise roughly 15\% of total switching costs \citep{CortesGallipoli2018}. The contribution is empirical validation methodology: demonstrating which distance constructions predict actual mobility, not proposing new distance concepts.

Additional module validation: The $\mathcal{I}$ module is validated---institutional distance adds predictive power conditional on semantic distance ($t = 34.6$). The $\mathcal{S}$ module is integrated---external AI exposure scores are orthogonal to task geometry ($r = 0.02$), confirming distinct constructs. The $\mathcal{M}$ module is calibrated externally: 3.84 wage-years per unit Wasserstein distance, anchored to structural estimates. Retrospective validation against the Autor-Dorn polarization setting confirms construct distinction: embedding geometry adds modest incremental explanatory power ($\Delta R^2 = 5.5\%$ for operator employment), consistent with the framework capturing mobility friction rather than automation susceptibility.

A central finding is the supply-demand decomposition: aggregate occupational inflows are demand-dominated ($\rho = 0.80$ for job openings alone), while geometry captures supply-side pathway direction (per-origin $\rho \approx 0.13$). This decomposition validates the oracle architecture's separation of where workers \emph{can} go (supply-side feasibility from skill geometry) from where they \emph{do} go (demand-side realization). The modular architecture enables cumulative progress; this paper establishes that the architecture is viable and empirically grounded.
\end{abstract}
...
A complete oracle would map $(\mathcal{T}, \mathcal{I}, \mathcal{S}, \mathcal{M}) \to (\Delta\boldsymbol{\rho}, \Delta \mathbf{L}, \Delta \mathbf{W})$: from inputs to occupation-specific changes in task distributions, employment, wages. No such oracle exists. This paper validates candidate specifications for $\mathcal{T}$ and $\mathcal{I}$, integrates external $\mathcal{S}$, and identifies what $\mathcal{M}$ requires.


The paper's contributions are framed as module validation checkpoints, with our primary contribution being \emph{empirical validation methodology} rather than conceptual innovation. Prior work applying embeddings to occupational descriptions (e.g., occ2vec, JobBERT) typically validates against O*NET's own similarity measures or expert ratings---potentially circular targets. We validate against revealed worker behavior: 89,329 actual CPS transitions.

First, the $\mathcal{T}$ module is validated through a 2$\times$2 methodology comparison isolating embedding choice from aggregation method. Embedding-based distances (using sentence embeddings of Detailed Work Activities) achieve 14.5\% pseudo-$R^2$ for predicting individual transitions, compared to 6--8\% for traditional O*NET-based measures---a 75\% relative improvement. The mechanism is the embedding ground metric: semantic task similarity captures that ``operating forklift'' $\approx$ ``driving delivery vehicle'' even as distinct task labels. Wasserstein optimal transport provides theoretical grounding as minimum semantic transformation cost; simpler centroid-averaging methods approximate this geometry closely ($\rho = 0.95$ correlation in distance rankings). The contribution is quantifying which distance constructions predict actual mobility, not proposing new distance concepts.

Second, the $\mathcal{I}$ module is validated: institutional distance (job zones plus certification requirements) adds predictive power conditional on embedding-based semantic distance ($\beta = 0.134$, $t = 32.8$). The separability assumption holds---skill barriers and credential barriers are distinct friction sources. Directional asymmetry (upward vs.\ downward barriers) is sample-dependent: ratio varies from 0.06 to 2.79 across specifications. Neither pure ``credential gate'' nor pure ``symmetric friction'' theories are universally supported; this heterogeneity constrains future $\mathcal{I}$ specifications.
...
Sixth, a gravity model analysis confirms that task distance explains a modest share of aggregate bilateral flow variation ($\sim$3.5\% partial $R^2$), consistent with \citet{CortesGallipoli2018}'s finding that task-specific costs account for approximately 15\% of structural switching costs. The divergence between individual choice prediction (where embeddings excel) and aggregate flow prediction (where simpler measures are competitive) reflects different margins: individual workers navigating among connected occupations use fine-grained task similarity, while aggregate patterns depend primarily on whether occupation pairs exchange workers at all.


The paper explores a model space defined by three axes: (i) \emph{representation}---how to encode tasks; (ii) \emph{geometry}---how to measure occupation distance; (iii) \emph{shocks}---how to characterize technology. Each empirical test constrains this space. The 2$\times$2 methodology comparison resolves representation: embedding-informed distance substantially outperforms O*NET-based measures. The RTI construct validity test establishes that semantic geometry is orthogonal to routine task intensity ($r = -0.05$)---the geometry captures mobility friction, not automation susceptibility. These are distinct constructs requiring distinct measurement.


Our approach complements structural estimation \citep{Traiberman2019} by trading parametric richness for applicability when administrative data is unavailable. We extend task-distance approaches \citep{GathmannSchonberg2010,CortesGallipoli2018} with embedding-informed geometry, and we extend embedding approaches \citep{ZhangEtAl2021,DecorteEtAl2022} with validation against revealed mobility rather than O*NET self-similarity. The supply-demand decomposition ($\rho \approx 0.80$ demand, $\rho \approx 0.13$ geometry) is consistent with \citet{CortesGallipoli2018}'s finding that task-related costs explain approximately 15\% of total transition frictions. The framework provides infrastructure for cumulative progress: validated modules can be reused as new capabilities are added.


\Cref{sec:literature} reviews related literatures organized by oracle module. \Cref{sec:theory} develops the oracle architecture and module specifications. \Cref{sec:data} operationalizes each component using O*NET data. \Cref{sec:validation} reports module validation results including geometry comparison, CPS mobility, shock integration, cost calibration, supply-demand decomposition, and aggregate flow analysis. \Cref{sec:extensions} documents extensions, completed investigations, and deferred work. \Cref{sec:limitations} documents scope conditions. \Cref{sec:applications} discusses policy applications and the research roadmap. \Cref{sec:conclusion} concludes with module status and next steps.
...
Gravity models of labor flows provide benchmarks for predictive accuracy. \citet{CortesGallipoli2018} use US CPS aggregate flows with DOT task measures, finding task-related costs explain $\leq$15\% of total transition costs. Their reduced-form task-only pseudo-$R^2$ was approximately 3\%, closely matching our gravity model estimates. \citet{SiminiEtAl2021} achieve correlations of 0.3--0.5 for geographic mobility predictions. These benchmarks contextualize our supply-demand decomposition.
...
\textbf{Identity ground metric} treats all tasks as equally different: $d(a, b) = \mathbf{1}[a \neq b]$. This is the implicit assumption of traditional O*NET-based measures that treat tasks as independent dimensions.
...
\begin{itemize}
    \item \textbf{Embedding effect:} +75\% improvement in pseudo-$R^2$ (embedding vs.\ O*NET methods)
    \item \textbf{Distributional effect:} +3\% improvement (Wasserstein vs.\ centroid on embeddings)
\end{itemize}
The embedding ground metric---semantic task substitutability---is the mechanism. Wasserstein provides theoretical grounding (minimum transformation cost) and modest empirical advantage; simpler centroid methods approximate the geometry closely ($\rho = 0.95$ correlation in distance rankings).
\end{remark}
...
\begin{itemize}
    \item O*NET-based measures: low correlation with institutional distance (largely orthogonal)
...
Occupation distance & \textbf{Wasserstein on embeddings} & Cosine on centroids, O*NET-based \\
...
\midrule
Embedding (MPNet) & Wasserstein & 8.95 & 14.51\% & +140\% \\
...
\begin{itemize}
    \item \textbf{Embedding effect:} Embedding-based methods (14.1--14.5\% pseudo-$R^2$) vs.\ O*NET-based methods (6.1--8.1\%) represents a +75\% relative improvement. This is the dominant effect.
    \item \textbf{Distributional effect:} Wasserstein (14.51\%) vs.\ cosine-on-centroids (14.08\%) represents a +3\% relative improvement. This is marginal.
\end{itemize}
...
\midrule
Identity (all tasks equally different) & 4.71 & 0.31 & 7.42\% & baseline \\
Embedding (semantic similarity) & 8.95 & 0.13 & 14.51\% & +96\% \\
\bottomrule
\end{tabular}
\end{table}


The embedding ground metric nearly doubles explanatory power (+96\%). Knowing that semantically similar tasks (e.g., ``operating forklift'' $\approx$ ``driving delivery vehicle'') are substitutable adds substantial predictive value beyond raw task overlap.
...
\textbf{Module status: $\mathcal{T}$ VALIDATED for embedding-informed distance.} The embedding ground metric (semantic task substitutability) is the validated mechanism. Wasserstein optimal transport provides theoretical grounding as minimum transformation cost and modest empirical advantage (+3\%); simpler centroid methods approximate the geometry closely. For practical applications where computational simplicity matters, cosine distance on embedding centroids may suffice.
...
\begin{table}[h!]
\centering
\caption{Conditional Logit Results: Embedding-Wasserstein (Primary)}
\label{tab:clogit-wasserstein}
\begin{tabular}{lcccc}
\toprule
\textbf{Parameter} & \textbf{Coefficient} & \textbf{Std. Error} & \textbf{$t$-statistic} & \textbf{$p$-value} \\
\midrule
$\alpha$ (semantic) & 8.953 & 0.043 & 206.5 & $< 10^{-100}$ \\
$\beta$ (institutional) & 0.134 & 0.004 & 32.8 & $< 10^{-100}$ \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Results:} Task distance explains a modest share of aggregate bilateral flow variation. The best-performing metric (embedding Wasserstein) achieves partial $R^2 = 3.46\%$. All distance coefficients are negative and highly significant: greater task distance reduces bilateral flows.

\textbf{Comparison to Cortes-Gallipoli:} Our partial $R^2$ values (0.25\%--3.46\%) are consistent with their finding that task-specific costs account for approximately 15\% of structural switching costs. Their reduced-form task-only pseudo-$R^2$ was approximately 3\%, closely matching our estimates. The dominant explanatory power in gravity models comes from origin and destination characteristics (captured by employment mass terms), not pairwise task distance.
...
\midrule
$\mathcal{T}$ & Operational & Embedding $\gg$ O*NET (+75\%) & Domain-specific embeddings \\
...
\begin{itemize}
    \item 2$\times$2 methodology comparison (\cref{subsec:geometry-comparison}): Embedding effect (+75\%) dominates distributional effect (+3\%)
    \item Ground metric validation (\cref{subsec:geometry-comparison}): Embedding ground metric +96\% over identity
    ...
    \item Gravity model analysis (\cref{subsec:gravity}): Partial $R^2 \approx 3.5\%$, consistent with Cortes-Gallipoli
\end{itemize}
...
The finding constrains future oracle specifications: geometry choice affects institutional inference, and directional barrier patterns should be tested for sample-dependence.
...
\textbf{Status: RESOLVED.} Embedding effect (+75\%) dominates distributional effect (+3\%). The embedding ground metric (semantic task substitutability) is the validated mechanism.
...
\midrule
Embedding $\gg$ O*NET & Semantic task substitutability matters & \textbf{Confirmed (+75\%)} \\
Wasserstein $\gg$ Centroid & Distributional treatment adds value & Marginal (+3\%) \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Implication:} Embedding-informed distance is adopted as primary $\mathcal{T}$ specification. Wasserstein provides theoretical grounding; simpler centroid methods approximate the geometry closely ($\rho = 0.95$).
...
\textbf{Status: DEFERRED.} With embedding-informed distance validated as primary geometry, domain-specific comparison is lower priority. The embedding effect (+75\%) is large; further embedding optimization unlikely to produce comparable improvement.
...
\textbf{Status: COMPLETED.} Embedding-based semantic exposure was correlated with Autor-Dorn RTI at occ1990dd level. Result: $r = -0.052$ (not significant).
...
\textbf{Status: COMPLETED.} Gravity model estimated on 199,362 occupation pairs. Embedding Wasserstein achieves partial $R^2 = 3.46\%$; O*NET cosine achieves 3.20\%. Results consistent with \citet{CortesGallipoli2018}.
...
\textbf{Module status:}
\begin{enumerate}
    \item \textbf{$\mathcal{T}$ module (task representation):} Validated. Embedding-informed distance substantially outperforms O*NET-based measures (+75\% in pseudo-$R^2$). The mechanism is semantic task substitutability: sentence embeddings capture that similar tasks are interchangeable even when distinctly labeled. Wasserstein optimal transport provides theoretical grounding (minimum transformation cost) and modest empirical advantage (+3\%); simpler centroid methods approximate the geometry closely ($\rho = 0.95$).
    
    \item \textbf{$\mathcal{I}$ module (institutional structure):} Validated. Institutional distance is associated with mobility conditional on semantic distance ($t = 32.8$). Separability holds. Directional asymmetry is sample-dependent---an informative constraint.
    ...
    \item \textbf{Scope:} Empirically quantified. Geometry captures supply-side pathway direction (per-origin $\rho \approx 0.13$); demand dominates aggregate flows ($\rho \approx 0.80$). Gravity model confirms task distance explains $\sim$3.5\% of aggregate bilateral flow variation, consistent with \citet{CortesGallipoli2018}. This decomposition is consistent with the oracle architecture's separation of feasibility from realization.
\end{enumerate}


\textbf{Informative constraints from this phase:}
\begin{itemize}
    \item Embedding ground metric matters: semantic task substitutability drives +75\% improvement over O*NET baselines
    \item Distributional treatment (Wasserstein) provides theoretical grounding but marginal empirical advantage (+3\%); centroid approximates well ($\rho = 0.95$)
    ...
    \item Task distance explains modest share of bilateral flows ($\sim$3.5\%), consistent with Cortes-Gallipoli (2018)
